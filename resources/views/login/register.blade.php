    ...
    <div class="flex justify-center items-center min-h-screen">
        <div class="container 2xl:w-[550px] 2xl:h-[760px] lg:w-[450px] lg:h-[590px] md:w-[400px] md:h-[560px] w-[300px] h-[480px] bg-white/30 backdrop-blur-sm rounded-[10px] shadow-md 2xl:py-14 py-8 px-10 border-[1px] border-white">
            ...
            <form action="{{ route('store') }}" method="POST" autocomplete="off">
                ...
                <input type="email" name="email" id="email" placeholder="Enter your Email address" class="form-control @error('email') is-invalid @enderror w-full 2xl:h-[50px] lg:h-[40px] md:h-[35px] h-[25px] text-[10px] md:text-sm bg-white/50 mb-5 px-5 backdrop-blur-sm border-[1px] border-white md:rounded-[10px] rounded-[5px]">
                @error('email')
                ...
                <div class="relative">
                    <input type="password" name="password" id="password" placeholder="Enter your Password" required class="form-control @error('password') is-invalid @enderror w-full 2xl:h-[50px] lg:h-[40px] md:h-[35px] h-[25px] md:rounded-[10px] rounded-[5px] text-[10px] md:text-sm bg-white/50 px-5 backdrop-blur-sm border-[1px] border-white">
                ...
                <button class="w-full lg:h-[40px] 2xl:h-[50px] md:h-[35px] h-[30px] md:rounded-[10px] rounded-[7px] bg-orange text-white 2xl:text-base text-[10px] md:text-sm font-normal btn btn-lg btn-primary" type="submit" >Sign in</button>
                </div>
            </form>
            ...
            <h1 class="block text-center 2xl:mt-5 lg:mt-3 md:mt-2 mt-1 text-grey 2xl:text-[15px] md:text-[13px] text-[10px]">Already have an account?<a class="text-dark" href="/login"><span class="underline underline-offset-1">Sign In </span></a></h1>
        </div>
    </div>
    <!-- Login Form End -->
</body>
